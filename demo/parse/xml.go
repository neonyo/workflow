...
func (p *xmlParser) Parse(ctx context.Context, content []byte) (*ParseResult, error) {
	...
	if version := process.SelectAttr("versionTag"); version != nil {
		result.FlowVersion, err = strconv.ParseInt(version.Value, 10, 64)
		if err != nil {
			return nil, err
		}
	}
	...
	for _, element := range process.ChildElements() {
		...
		if element.Tag == "subProcess" {
			for _, vv := range element.ChildElements() {
				if vv.Tag == "documentation" ||
					vv.Tag == "extensionElements" ||
					vv.Tag == "sequenceFlow" || vv.Tag == "incoming" || vv.Tag == "outgoing" {
					continue
				}
				//fmt.Println(vv.Tag, "===---\"")
				subNode, _ := p.ParseNode(vv)
				var subNodeResult NodeResult
				subNodeResult.NodeID = subNode.Code
				subNodeResult.NodeName = subNode.Name
				subNodeResult.NodeType, err = GetNodeTypeByName(subNode.Type)
				if err != nil {
					return nil, err
				}
				fmt.Println(subNodeResult, "1122")
			}
		}
